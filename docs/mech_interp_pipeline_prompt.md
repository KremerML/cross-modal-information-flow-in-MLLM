# Pipeline prompt: Mech Interp × Multimodal AI Research Direction Identification

## Context

I'm a first-year PhD candidate at ILLC (University of Amsterdam, Mulini Lab), researching mechanistic interpretability of vision-language models. I want to systematically explore where applied mech interp could matter — inspired by Goodfire's applied direction.

You will follow a structured problem→idea pipeline that I developed. The goal is to surface candidate research directions. Outputs are raw food for thought; whether I pursue any of them depends on quality.
...
**Explicit non-goals:** I'm not asking what mech interp tools should do (architectural research). I'm asking where they should be applied (problem identification across applied domains).
...
**Push back on diminishing returns.** If you think the back half of a 30-item list will be padding, say so before writing. Quality over quantity.
...
**Tone.** Honest, slightly skeptical, willing to disagree. I'm not looking for affirmation; I'm looking for a thinking partner who flags weaknesses. This pipeline works because we catch real errors in iteration. Apply the same standard here.
